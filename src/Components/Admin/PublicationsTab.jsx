...
import { Package, Search, MapPin, Calendar, Clock, Trash2, ArrowRight, Zap, PauseCircle, PlayCircle } from 'lucide-react';
import { Pagination } from './Pagination';

export const PublicationsTab = ({ pubsFilter, setPubsFilter, filteredPubs, pagedPubs, pagePubs, setPagePubs, ITEMS_PER_PAGE, handleDeletePublication, handleStatusToggle, safeDateStr }) => (
            ...
            <table className="w-full text-left">
                ...
                <tbody className="divide-y divide-slate-100">
                    {pagedPubs.length > 0 ? pagedPubs.map(pub => {
                        ...
                        const isCompleted = pub.status === 'completed';
                        
                        return (
                        <tr key={pub.id} className={`transition-colors ${isPaused ? 'bg-amber-50/30 hover:bg-amber-50/60' : 'hover:bg-slate-50/50'}`}>
                            <td className="p-5">
                                ...
                                <p className="text-[10px] text-slate-500 font-mono mt-0.5 bg-slate-100 px-1.5 py-0.5 rounded w-max border border-slate-200">ID: {pub.customId || pub.id.substring(0,6)}</p>
                            </td>
                            <td className="p-5">
                            ...
                            <td className="p-5">
                                ...
                                <span className={`inline-block mt-1.5 px-2 py-0.5 rounded-md text-[9px] font-bold border ${pub.status === 'active' ? 'bg-emerald-50 text-emerald-600 border-emerald-100' : pub.status === 'completed' ? 'bg-blue-50 text-blue-600 border-blue-100' : 'bg-amber-50 text-amber-600 border-amber-200'}`}>
                                    {pub.status === 'active' ? 'ACTIVA' : pub.status === 'paused' ? 'PAUSADA' : pub.status.toUpperCase()}
                                </span>
                            </td>
                                ...
                                <div className="flex items-center justify-end gap-2">
                                    {!isCompleted && (
                                        <button 
                                            ...
                                            className={`p-2.5 rounded-xl border transition-colors shadow-sm ${pub.status === 'active' ? 'bg-white border-amber-200 text-amber-500 hover:bg-amber-50 hover:text-amber-600' : 'bg-emerald-50 border-emerald-200 text-emerald-600 hover:bg-emerald-100 hover:text-emerald-700'}`} 
                                            title={pub.status === 'active' ? 'Pausar Publicación' : 'Activar Publicación'}
                                        >
                                            {pub.status === 'active' ? <PauseCircle size={16}/> : <PlayCircle size={16}/>}
                                        </button>
                                    )}
                                    <button 
                                        onClick={() => handleDeletePublication(pub.id, pub.type)} 
                                        className="p-2.5 bg-white border border-rose-200 text-rose-500 rounded-xl hover:bg-rose-50 hover:text-rose-600 transition-colors shadow-sm" 
